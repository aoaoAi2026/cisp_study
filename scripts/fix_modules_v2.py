"""
稳健修复脚本 v2 - 为三个文件添加缺失的 resources、recommendedTools、labEnvironment
使用 day id 标记精确定位，不会插入到课程内容中
"""
import json, re, os
...
def fix_file(filepath):
    ...
    # 判断文件类型
    if 'basic-' in ''.join(day_ids[:3]):
        ftype = 'basic'
    elif 'def-' in ''.join(day_ids[:3]):
        ftype = 'defense'
    else:
        ftype = 'penetration'
    ...
    for i, day_id in enumerate(day_ids):
        ...
        # 找当前day的起止位置
        day_start = content.find(f"id: '{day_id}'")
        if day_start < 0:
            continue
        
        # 往回找 '{' 作为 day 对象开始
        obj_start = content.rfind('{', 0, day_start)
        if obj_start < 0:
            continue
        ...
        # 从obj_start开始找匹配的}
        # 先找下一个 id: 标记作为参考上限
        next_id = re.search(r"\n\s*\{\s*\n\s*id:\s*'", content[day_start + 50:])
        if next_id:
            search_end = day_start + 50 + next_id.start()
        else:
            search_end = len(content)
        
        # 在search_end之前找day对象结束的'  }\n'
        # 策略: 在search_end之前找 '  },' 或 '  }\n' 且在检查范围内
        segment = content[obj_start:search_end]
        # 找 '  },' 或直接 '  }' 后面跟 ','
        end_patterns = [r'\n\s+\}(?=\s*,?\s*\n\s*(?:\{|\]))', r'\n\s+\}(?=\s*$)']
        day_end_in_segment = None
        for pat in end_patterns:
            m = re.search(pat, segment)
            if m:
                day_end_in_segment = m.start()
                break
        
        if day_end_in_segment is None:
            continue
        
        day_end = obj_start + day_end_in_segment
        ...
        if to_add:
            new_text = content[:day_end] + to_add + content[day_end:]
            content = new_text
            modified_count += 1
            print(f'  {rid}: +{to_add.count(": [")} modules')
...
if __name__ == '__main__':
    print('=== 修复 Basic ===')
    n = fix_file(os.path.join(BASE, 'src/data/cyberBasic.ts'))
    print(f'  Basic: {n} 天修改')

    print('\n=== 修复 Defense ===')
    n = fix_file(os.path.join(BASE, 'src/data/cyberDefense.ts'))
    print(f'  Defense: {n} 天修改')

    print('\n=== 修复 Penetration ===')
    n = fix_file(os.path.join(BASE, 'src/data/cyberPenetration.ts'))

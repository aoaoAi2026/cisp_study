# -*- coding: utf-8 -*-
"""Smart rich generator - compact data -> 500+ line .md files. ALL raw strings."""
import os
...
def expand_sec(title, body):
    """Section with title and body (body is raw string markdown)"""
    return f'\n### {title}\n\n{body}\n'
...
def make_file(day, title, diff, goals, intro_text, sections, tasks, checklist, summary, reading, next_title=''):
    ...
    for i, (stitle, sbody) in enumerate(sections, 1):
        body += expand_sec(f'{cn(i)}、{stitle}', sbody)
    body += FOOTER.format(
...
# ============ CONTENT DATA (ALL raw strings r"""...""") ============

# Day 3: HTTP/HTTPS Protocol
WL_day3 = lambda: write_file(3, 'HTTP/HTTPS协议核心——Web安全的基石', '⭐ 入门', [
    '完全理解 HTTP 请求和响应的结构——请求行、请求头、请求体各是什么',
    '记住 5 种最重要的 HTTP 状态码及其含义——看到一个数字立刻知道发生了什么',
    '彻底分清 GET 和 POST 的区别——不只是「一个能看到参数一个看不到」',
    '理解 HTTPS 加密的工作原理（SSL/TLS 握手全过程）',
    '学会用浏览器 F12 和 Wireshark 实战分析 HTTP 流量',
    '能够从 HTTP 层面识别常见的 Web 攻击特征'
], r'''
> 🧠 **白话理解**：HTTP 就像你去餐厅吃饭的「语言」——你告诉服务员「我要点菜」（请求），服务员端菜上来（响应）。HTTPS 就是在这基础上加了「密室包厢」——你们说的话外面听不见。今天彻底搞懂浏览器和服务器之间到底在「说」什么。
''', [
    ('HTTP是什么——用「去餐厅吃饭」彻底搞懂', r'''
...
'''),
    ...
    ('5 个最重要的 HTTP 状态码——见码知意', r'''
服务器用三位数字告诉你请求的结果。蓝队必须一眼认出：

''' + gentbl(['状态码', '含义', '打个比方', '蓝队看到它的反应'], [
...
'''),
    ...
    ('GET vs POST——不只是「能不能看到参数」', r'''
很多教程说「GET 参数在 URL 里能看见，POST 在请求体里看不见」。这话对但不全对。

''' + gentbl(['区别维度', 'GET', 'POST'], [
...
'''),
    ...
    ('HTTPS 加密原理——SSL/TLS 握手全过程（白话版）', r'''
...
'''),
    ...
    ('Cookie、Session、Token——认证三兄弟彻底分清', r'''
面试必考，而且很多人一辈子都分不清。用火车站的比喻一次性讲明白：

''' + gentbl(['概念', '火车站比喻', '技术实现'], [
...
'''),
    ...
    '打开浏览器 F12 → Network 标签 → 刷新 baidu.com → 逐个点击请求查看 Headers/Preview/Timing',
    '找一个 POST 请求，观察请求头和请求体中分别包含什么信息',
    '访问 https://www.ssllabs.com/ssltest/ 测试一个网站的 SSL 评级',
    '在浏览器地址栏手动输入不存在的页面路径 → 观察返回的 404 状态码',
    '找一个有登录功能的网站 → F12 抓取登录请求 → 查看 Payload 中提交的数据格式',
], [
...
], r'''今天学会了 HTTP 的「骨架」——请求行/请求头/请求体三元结构，5 种核心状态码，GET/POST 深层区别，HTTPS 加密原理。最关键的是掌握了从 HTTP 层面「读」攻击——SQL 注入的引号和 OR、XSS 的 script 标签、扫描器的异常 UA 和大批量 404。这些是蓝队日常分析中最高频的场景。''', [
...
], 'Day 4：Linux系统基础（上）——文件管理+用户权限+基础命令')

# ============ MAIN ============

if __name__ == '__main__':
    lines = WL_day3()
    print(f'Day 3: {lines} lines')
    print('Done!')

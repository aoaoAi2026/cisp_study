"""
稳健修复脚本 v3 - 为三个文件添加缺失的 resources、recommendedTools、labEnvironment
使用括号计数法精确定位 day 对象边界，兼容单行和分行两种格式
"""
import json, re, os
...
def init_data():
    ...
    # Defense labs - def-2 through def-30
    labs = [
...
def find_day_closing_brace(content, obj_start):
    ...
    while i < len(content):
        c = content[i]
        c_prev = content[i-1] if i > 0 else ''
        ...
        # 处理字符串
        if c == "'" and not in_double and not in_backtick and not in_comment_line and not in_comment_block:
            # 检查转义
            escaped = False
            j = i - 1
        ...
        elif c == '"' and not in_single and not in_backtick and not in_comment_line and not in_comment_block:
            escaped = False
            j = i - 1
            while j >= 0 and content[j] == '\\':
                escaped = not escaped
        ...
        elif c == '`' and not in_single and not in_double and not in_comment_line and not in_comment_block:
            # 模板字面量 - 不处理转义，保持简单
...
def fix_file(filepath):
    ...
    for m in re.finditer(r"id:\s*'([^']+)'", content):
        ...
        if did.startswith('basic-') or did.startswith('def-') or did.startswith('pen-'):
            ...
            if obj_start >= 0:
                day_ids.append((did, obj_start, m.start()))

    # 判断文件类型
    ftype = 'basic'
    if day_ids:
        first_id = day_ids[0][0]
        if first_id.startswith('def-'):
            ftype = 'defense'
        elif first_id.startswith('pen-'):
            ftype = 'penetration'
    ...
    for day_id, obj_start, id_pos in day_ids:
        # 找到匹配的结束 }
        day_end = find_day_closing_brace(new_content, obj_start)
        if day_end < 0:
            print(f'  {day_id}: 无法找到day对象结束位置，跳过')
            continue
        ...
        if rid in RESOURCE_MAP and not has_res:
            to_add += ',\n      resources: ' + json.dumps(RESOURCE_MAP[rid], ensure_ascii=False)

        # 2. recommendedTools (仅 Defense)
        has_tools = 'recommendedTools:' in day_text
        if ftype == 'defense' and rid in DEFENSE_TOOLS and not has_tools:
            to_add += ',\n      recommendedTools: ' + json.dumps(DEFENSE_TOOLS[rid], ensure_ascii=False)

        # 3. labEnvironment (仅 Defense, def-2+)
        has_lab = 'labEnvironment:' in day_text
        if ftype == 'defense' and rid in DEFENSE_LABS and not has_lab:
            to_add += ',\n      labEnvironment: [' + json.dumps(DEFENSE_LABS[rid], ensure_ascii=False) + ']'
        ...
        if to_add:
            ...
            modified_count += 1
            details = []
            if 'resources' in to_add: details.append('resources')
            if 'recommendedTools' in to_add: details.append('tools')
            if 'labEnvironment' in to_add: details.append('lab')
            print(f'  {day_id}: +{", ".join(details)}')

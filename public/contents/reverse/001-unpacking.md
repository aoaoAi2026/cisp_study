...
> **📘 文档定位**：CISP 考试逆向工程基础内容 | 难度：⭐⭐⭐⭐ | 预计阅读：20 分钟
> 脱壳是逆向工程的基础技能。本文从壳的分类、三步脱壳法到常见反调试对抗，系统梳理脱壳技术体系。
...
> 🔑 **高分考点**：脱壳技术考点集中在壳的分类、三步脱壳法(OEP→Dump→IAT)、反调试对抗。

| 考点 | 频次 | 核心记忆点 |
|:---|:---:|:---|
| 三步脱壳法 | ⭐⭐⭐⭐⭐ | 找 OEP → Dump 内存 → 修复 IAT |
| ESP 定律 | ⭐⭐⭐⭐ | pushad 后在 ESP 指向栈内存下硬件断点 |
| 壳的分类 | ⭐⭐⭐⭐ | 压缩壳(UPX)→加密壳(ASProtect)→虚拟化(VMP) |

> 💡 **知识巧记**：脱壳三步记"找 O 倒 I"——OEP 定位、Dump 导出、IAT 修复。ESP 定律记"pushad 后栈断点，popad 近 OEP 现"。

### 高分考点速查表

| 考察维度 | 关键结论 | 常见干扰项 |
|:---|:---|:---|
| UPX 脱壳 | upx -d 直接解压，修改版需动态调试 | "所有 UPX 都能 upx -d" ❌ |
| Stolen Bytes | OEP 附近代码被壳偷走，需手工回填 | "找到 OEP 就能直接 dump" ❌ |

### 知识巧记口诀

> **脱壳口诀**：
> 三步脱壳找 O 倒 I，ESP 定律 pushad 栈断点。
> 压缩直接 upx -d，加密 ESP 定律跟。
> VMP 虚拟化无 OEP，trace 黑盒分析 API 边界。

...
> **📘 文档定位**：CISP 考试云安全核心基础 | 难度：⭐⭐⭐⭐ | 预计阅读：25 分钟
> 云安全是安全责任共担模型下的新战场。本文从云安全架构、渗透测试要点、IAM 权限提升、云存储安全到容器逃逸，系统梳理云安全的攻击与防御体系。

---

## 导航目录
- [一、云安全责任共担模型](#1-云安全责任共担)
- [二、云渗透测试要点](#2-云渗透测试要点)
- [三、IAM权限提升](#3-iam权限提升)
- [四、S3/OSS 存储安全](#4-s3oss-存储安全)
- [五、容器与K8s安全](#5-容器与k8s安全)
- [六、云日志与监控](#6-云日志与监控)
...
| 序号 | 考点 | 频率 | 难度 | 关键答案 |
|:---:|:---|:---:|:---:|:---|
| 1 | 云安全责任共担模型 | ⭐⭐⭐⭐⭐ | ⭐⭐ | SaaS全厂商负责、PaaS厂商底层用户应用、IaaS厂商物理用户OS以上 |
| 2 | 元数据服务攻击 | ⭐⭐⭐⭐⭐ | ⭐⭐ | SSRF访问 169.254.169.254 获取IAM角色临时凭证 |
| 3 | 云安全七大配置错误 | ⭐⭐⭐⭐ | ⭐⭐ | S3公开、0.0.0.0/0、IAM过大、AK/SK硬编码、IMDSv1、数据库公开、K8s暴露 |
| 4 | IAM PassRole 提权 | ⭐⭐⭐⭐ | ⭐⭐⭐ | ec2:RunInstances+iam:PassRole → 创建EC2传递高权限角色 |
| 5 | 容器逃逸三种方式 | ⭐⭐⭐⭐ | ⭐⭐⭐ | 特权容器挂载宿主机、挂载docker.sock、crontab写反弹Shell |
| 6 | 云日志审计关键事件 | ⭐⭐⭐ | ⭐⭐ | 异常地域API、非工作时间操作、新IAM创建、安全组0.0.0.0修改 |

### 知识巧记口诀
> 🎵 **云安全七宗罪**："S3公开第一罪，安全组全开第二罪；IAM过大第三罪，密钥硬编码第四罪；元数据未防护第五罪，数据库裸奔第六罪；K8s暴露第七罪——七罪齐防保云安"

> 🎵 **云渗透三把刀**："元数据窃凭证、PassRole提权限、docker.sock逃逸——三刀拿下云主机"

...
| 考点 | 考察维度 | 记忆要点 |
|------|----------|----------|
| 信息收集七阶段 | 流程记忆 | 被动→DNS→子域名→端口→Web指纹→JS分析→目录爆破 |
| 子域名收集工具链 | 工具选型 | 证书透明度(crt.sh)、DNS暴力(subfinder)、搜索引擎(shodan) |
| CDN绕过技术 | 实战技巧 | 历史DNS、SSL证书、子域名、邮件头、国外节点 |
| 指纹识别维度 | 综合分析 | CMS(whatweb)、WAF(wafw00f)、中间件(httpx)、JS库特征 |
| JS信息泄露挖掘 | 代码审计 | API端点、内网IP、AK/SK、Token、隐藏路径 |
| 资产测绘输出 | 标准化 | 攻击面分析图→资产清单表→风险优先级排序 |

### 知识巧记口诀

> **信息收集顺口溜**：
> 被动收集先开始，Whois证书和搜索引擎；
> DNS枚举多工具，A记录MX加CNAME；
> 端口扫描nmap跑，全端口加版本号；
> Web指纹要准确，CMS框架和中间件；
> JS源码仔细看，API密钥和隐藏链接；
> 目录爆破字典好，.git/.env别忘了！

> **CDN绕过口诀**：历史DNS找真实IP，SSL证书搜全网域名，子域名可能没套CDN，邮件头泄露源站地址。

### 考试陷阱提醒

| 陷阱 | 正确认知 |
|------|----------|
| ❌ 信息收集=只用nmap | ✅ 信息收集是多维度多工具的交叉验证过程 |
| ❌ CDN后无法获取真实IP | ✅ 多种绕过技术可获取源站IP（历史DNS/SSL证书等） |
| ❌ 子域名收集不重要 | ✅ 子域名往往是攻破主站的"旁路"，CISP考试高频考点 |

> 💡 **一句话总结**：信息收集决定渗透测试的成败——收集越全面，攻击面越清晰，成功概率越高。CISP考试重点考查信息收集的完整流程和工具链选择。

...
网络安全分析师每天面对海量数据。假设检验与统计推断是将这些分散的数据点转化为可操作安全洞察的核心技术。
...
学习路径中的定位：
  ...
  安全基础(1-21) -> 数据工程(22-35) -> 经典ML(36-56) -> [假设检验与统计推断] -> 下一步
  积累了安全知识     掌握了数据处理    学会了ML建模    本日核心    继续前进
```

### 本日学习目标

1. 深入理解假设检验与统计推断的核心原理和数学基础
2. 掌握假设检验与统计推断在Python中的完整实现
3. 在真实网络安全数据集上实践假设检验与统计推断
4. 理解假设检验与统计推断在生产环境中的注意事项与陷阱
...
学习假设检验与统计推断不只是掌握算法本身，更重要的是理解其在安全工程中的定位：
...
```text
假设检验与统计推断在安全ML系统中的位置：

  安全数据 -> 特征工程 -> [假设检验与统计推断] -> 安全决策 -> SOC响应
     |           |           |            |           |
...
|------|------|------|------|
| 卡方检验 | 独立性检验 | 分类变量关系 | ★★★★ |
| KS检验(Kolmogorov-Smirnov) | 分布差异检验 | 攻击vs正常分布 | ★★★★★ |
| Mann-Whitney U | 非参数检验 | 不假设分布 | ★★★★ |
| p值与显著性水平 | 假设检验核心 | 误报风险权衡 | ★★★★★ |
| 多重检验校正 | Bonferroni/FDR | 大规模特征筛选 | ★★★★ |
...
### 场景1：假设检验与统计推断在SOC日常运营中的应用

某大型企业的SOC每天处理来自50+安全设备的告警。引入假设检验与统计推断技术后：
- 告警处理效率显著提升
- 误报率大幅下降
...
而假设检验与统计推断能够通过学习正常行为模式来发现偏离——不管是已知还是未知攻击。
...
1. **影子部署**：先在非关键系统运行假设检验与统计推断模型1个月，收集性能数据
...
请简述假设检验与统计推断的核心原理，以及在网络安全场景中的典型应用步骤。
...
你被要求设计一个基于假设检验与统计推断的网络安全检测系统。请从数据准备、模型设计、评估策略、部署方案和运维监控五个方面给出完整方案。
...
1. 构建包含假设检验与统计推断的完整安全检测Pipeline
...
| 知识点 | 核心内容 | 一句话记忆 |
|------|------|------|
| 假设检验与统计推断原理 | 理解核心机制和数学基础 | 掌握本质才能灵活应用 |
...
> **📌 下节预告**：Day 19 学习**统计异常检测：Z-Score/IQR/MAD**。
> 

...
> 代码签名是软件供应链完整性的核心技术。本文从签名原理、Sigstore/Cosign 方案到验证流程，系统梳理代码签名实践。
...
> 🔑 **高分考点**：代码签名考点集中在 Sigstore/Cosign 方案、签名验证流程、与 SLSA 的关系。

| 考点 | 频次 | 核心记忆点 |
|:---|:---:|:---|
| Cosign | ⭐⭐⭐⭐ | Sigstore 项目，无密钥签名+透明日志(Rekor) |
| 签名验证 | ⭐⭐⭐⭐ | 推送签名→部署前验证，准入控制器拦截 |
| 密钥管理 | ⭐⭐⭐ | OIDC 短期密钥替代长期私钥 |

> 💡 **知识巧记**：Cosign 三件套记"Co 签 Re 录 Ful 证"——Cosign 签名、Rekor 透明日志、Fulcio 证书。

### 高分考点速查表

| 考察维度 | 关键结论 | 常见干扰项 |
|:---|:---|:---|
| 无密钥签名 | OIDC token + Fulcio 签发短期证书 | "Cosign 需要管理长期私钥" ❌ |
| Rekor | 透明日志，不可篡改的签名记录 | "Rekor 是私钥存储服务" ❌ |

### 知识巧记口诀

> **代码签名口诀**：
> Cosign 无钥签名新范式，OIDC+Fulcio 短期证。
> Rekor 透明日志不可篡改，准入控制器拦未签。

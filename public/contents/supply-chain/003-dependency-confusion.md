...
> **📘 文档定位**：CISP 考试供应链安全技术内容 | 难度：⭐⭐⭐⭐ | 预计阅读：20 分钟
> 依赖混淆攻击是利用包管理器命名解析机制的供应链攻击。本文从攻击原理、典型案例到防御方案，系统梳理依赖混淆攻防。

---

## 导航目录
- [一、依赖混淆攻击原理](#一依赖混淆攻击原理)
...
> 🔑 **高分考点**：依赖混淆考点集中在攻击原理、Alex Birsan 案例、防御方案。

| 考点 | 频次 | 核心记忆点 |
|:---|:---:|:---|
| 依赖混淆原理 | ⭐⭐⭐⭐⭐ | 私有包名与公共仓库同名，包管理器优先拉取高版本公共包 |
| Alex Birsan | ⭐⭐⭐⭐ | 2021 年利用依赖混淆攻破 Apple/Microsoft/PayPal 等 35 家企业 |
| 防御方案 | ⭐⭐⭐⭐ | 私有仓库优先、scoped packages、包名前缀、版本锁定 |

> 💡 **知识巧记**：依赖混淆记"私名公仓同，高版优先拉"。防御记"私有优先 scope 隔离"。

### 高分考点速查表

| 考察维度 | 关键结论 | 常见干扰项 |
|:---|:---|:---|
| npm scope | @company/package 可防止公有仓库同名 | "scope 完全解决依赖混淆" ❌ |
| 版本策略 | 锁定内部包版本号，禁止自动升级 | "自动升级到最新版本最安全" ❌ |

### 知识巧记口诀

> **依赖混淆口诀**：
> 私名公仓同高版优先拉，Alex Birsan 三十五企破。
> 防御私有优先 scope 隔离，版本锁定不自动升。

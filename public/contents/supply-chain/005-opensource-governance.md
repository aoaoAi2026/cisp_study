...
> **📘 文档定位**：CISP 考试供应链安全治理内容 | 难度：⭐⭐⭐ | 预计阅读：18 分钟
> 开源组件治理是现代软件安全的基础。本文从组件识别、漏洞管理到许可证合规，系统梳理开源安全治理体系。

---

## 导航目录
- [一、开源治理面临的挑战](#一开源治理面临的挑战)
...
> 🔑 **高分考点**：开源治理考点集中在组件识别、漏洞管理、许可证合规。

| 考点 | 频次 | 核心记忆点 |
|:---|:---:|:---|
| 组件识别 | ⭐⭐⭐⭐ | SCA 工具自动扫描依赖树，识别直接/传递依赖 |
| 漏洞管理 | ⭐⭐⭐⭐ | CVE/NVD 匹配，CVSS 评分，修复优先级排序 |
| 许可证合规 | ⭐⭐⭐ | GPL/copyleft 风险、商业兼容性审查 |

> 💡 **知识巧记**：开源治理三步记"识漏许"——识别组件、漏洞管理、许可证合规。

### 高分考点速查表

| 考察维度 | 关键结论 | 常见干扰项 |
|:---|:---|:---|
| 传递依赖 | 间接依赖也可能引入漏洞，需全量扫描 | "只扫描直接依赖即可" ❌ |
| GPL 传染性 | GPL 许可证可能要求衍生作品也开源 | "所有开源许可证都无限制" ❌ |

### 知识巧记口诀

> **开源治理口诀**：
> 识漏许三步走，SCA 全量扫直接传递。
> CVE 匹配 CVSS 排序，GPL 传染性需警惕。

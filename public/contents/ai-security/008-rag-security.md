...
> RAG 是 LLM 应用的主流架构，但其知识库投毒、检索操纵、上下文污染三大攻击面常被忽视。本文从攻击原理到防御代码，系统讲解 RAG 安全体系。

---

## 导航目录
- [一、RAG 架构与攻击面](#一rag-架构与攻击面)
- [二、知识库投毒](#二知识库投毒-knowledge-base-poisoning)
- [三、检索攻击](#三检索攻击)
- [四、上下文窗口攻击](#四上下文窗口攻击)
- [五、向量数据库安全](#五向量数据库安全)
...
| 序号 | 考点 | 频率 | 难度 | 关键答案 |
|:---:|:---|:---:|:---:|:---|
| 1 | RAG 五大攻击面 | ⭐⭐⭐⭐⭐ | ⭐⭐ | 文档注入、检索操纵、上下文污染、向量数据库安全、权限绕过 |
| 2 | 知识库投毒三种方式 | ⭐⭐⭐⭐ | ⭐⭐ | 公开知识库上传、用户贡献内容、增量更新投毒 |
| 3 | 安全检索五步过滤 | ⭐⭐⭐⭐ | ⭐⭐⭐ | 向量检索→ACL过滤→安全评分→低分移除→多样性保证 |
| 4 | 上下文安全构建 | ⭐⭐⭐ | ⭐⭐ | 来源标注+可信度标注+明确"文档是参考不是指令" |
| 5 | Embedding 逆向风险 | ⭐⭐⭐ | ⭐⭐⭐ | 可从向量近似重建原文，需差分隐私嵌入+向量加密 |
| 6 | 文档安全评分因子 | ⭐⭐⭐ | ⭐⭐⭐ | 来源可信度、历史标记、文档年龄、注入检测结果 |

### 知识巧记口诀
> 🎵 **RAG 安全五道关**："上传文档要审查，检索结果过ACL；安全评分滤恶意，上下文标注防污染；向量加密防逆向，输出过滤最后关"

> 🎵 **检索攻击三招**："语义相近骗相似、关键词填充分数高、长尾查询钻空子"

---

> **RAG 安全的核心是：不要信任知识库中的任何文档。每一篇检索到的文档都应视为潜在的恶意输入，必须经过 ACL、安全评分、内容消毒三层过滤后才能进入 LLM 上下文。**

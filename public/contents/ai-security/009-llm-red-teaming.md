...
> LLM 红队测试是验证 AI 安全防护有效性的关键手段。本文从测试框架选型、四维测试矩阵、Garak/PyRIT 实战到红队报告模板，提供完整的 LLM 安全评估方法论。

---

## 导航目录
- [一、LLM 红队测试框架](#一llm-红队测试框架)
- [二、测试维度与用例设计](#二测试维度与用例设计)
- [三、Garak 实战](#三garak-实战)
- [四、PyRIT 微软红队工具](#四pyrit-微软红队工具)
- [五、红队报告模板](#五红队报告模板)
...
| 序号 | 考点 | 频率 | 难度 | 关键答案 |
|:---:|:---|:---:|:---:|:---|
| 1 | LLM 红队测试四维矩阵 | ⭐⭐⭐⭐⭐ | ⭐⭐ | 安全（注入/越狱）、伦理（偏见/有害）、合规（PII/备案）、鲁棒性（对抗/幻觉） |
| 2 | Garak 核心能力 | ⭐⭐⭐⭐ | ⭐⭐ | 支持20+探针类型，覆盖注入/越狱/数据泄露/恶意代码生成 |
| 3 | PyRIT vs Garak | ⭐⭐⭐ | ⭐⭐⭐ | PyRIT：微软出品，编排能力强；Garak：社区开源，探针丰富 |
| 4 | 越狱测试重点 | ⭐⭐⭐⭐ | ⭐⭐ | DAN模式、Base64编码绕过、多语言混合、逐步引导 |
| 5 | 红队报告核心要素 | ⭐⭐⭐ | ⭐⭐ | 评估概述→测试方法→关键发现（分级）→修复建议→结论 |
| 6 | 多语言安全对齐 | ⭐⭐⭐ | ⭐⭐⭐ | 小语种安全限制可能弱于主流语言，需全面覆盖 |

### 知识巧记口诀
> 🎵 **红队测试四维**："安全注入和越狱，伦理偏见与有害；合规备案防泄露，鲁棒对抗抗幻觉——四维全测才算完"

> 🎵 **Garak 四类探针**："注入越狱是A类，内容安全算B类；数据泄露归C类，对齐测试在D类"

---

> **LLM 红队测试不是一次性活动，而应纳入 CI/CD 流程。每个新版本发布前，自动化+人工红队测试必须通过，安全得分达标方可上线。**

...
> PEFT 参数高效微调（LoRA/QLoRA）让模型定制变得极其简单，但也打开了后门攻击的新大门。本文从 LoRA 后门攻击原理、恶意权重检测、激活聚类分析到防御体系，全面剖析微调安全。

---

## 导航目录
- [一、PEFT 微调生态](#一peft-微调生态)
- [二、LoRA 后门攻击](#二lora-后门攻击)
- [三、CivitAI / 社区 LoRA 风险](#三civitai--社区-lora-风险)
- [四、后门防御技术](#四后门防御技术)
...
| 序号 | 考点 | 频率 | 难度 | 关键答案 |
|:---:|:---|:---:|:---:|:---|
| 1 | LoRA 为何成为新攻击面 | ⭐⭐⭐⭐⭐ | ⭐⭐ | 仅需几MB、单卡可训练、社区平台无审查——门槛极低 |
| 2 | LoRA 后门两种模式 | ⭐⭐⭐⭐ | ⭐⭐ | 触发词后门（特定token触发）和领域后门（特定领域触发） |
| 3 | LoRA 权重文件大小 | ⭐⭐⭐ | ⭐⭐ | Llama-7B全量~13GB，LoRA(r=8)仅~8MB，极易分发 |
| 4 | 恶意LoRA检测方法 | ⭐⭐⭐ | ⭐⭐⭐ | 权重极端值检测、激活聚类分析、行为测试 |
| 5 | CivitAI 安全风险 | ⭐⭐⭐ | ⭐⭐⭐ | 仅基础NSFW标签，无后门/代码安全扫描，完全依赖社区 |
| 6 | PEFT vs 全量微调安全性 | ⭐⭐⭐ | ⭐⭐ | PEFT门槛低但供应链不可控，全量微调成本高但相对可控 |

### 知识巧记口诀
> 🎵 **LoRA 后门检测**："权重极端查异常，激活聚类找后门；行为测试验触发，safetensors 格式保安全"

> 🎵 **微调安全守则**："来源要可信、格式要安全、沙箱先测试、行为再验证"

---

> **LoRA 微调是 LLM 生态的双刃剑——几 MB 的权重文件即可定制模型能力，也可能植入难以察觉的后门。永远不要加载来源不明的 LoRA 权重，这条原则和"不要运行未知来源的可执行文件"同等重要。**

...
> LLM Agent 具备工具调用和自主决策能力，风险面远超传统 LLM。本文深入剖析 Agent 六大攻击面、Function Calling 注入、Copilot 类产品安全风险及沙箱隔离最佳实践。

---

## 导航目录
- [一、LLM Agent 安全模型](#一llm-agent-安全模型)
- [二、工具调用注入攻击](#二工具调用注入攻击)
- [三、Copilot 类产品安全](#三copilot-类产品安全)
- [四、Agent 权限控制最佳实践](#四agent-权限控制最佳实践)
...
| 序号 | 考点 | 频率 | 难度 | 关键答案 |
|:---:|:---|:---:|:---:|:---|
| 1 | Agent 六大攻击面 | ⭐⭐⭐⭐⭐ | ⭐⭐ | 工具注入、函数污染、间接注入、权限过度、自主失控、无限循环 |
| 2 | Agent vs 传统 LLM 核心区别 | ⭐⭐⭐⭐⭐ | ⭐⭐ | Agent 会"思考→行动→观察→再思考"循环，不仅"说"还会"做" |
| 3 | Function Calling 注入 | ⭐⭐⭐⭐ | ⭐⭐⭐ | 用户输入诱导 Agent 调用危险工具函数，需操作白名单+参数校验 |
| 4 | Copilot 三大风险 | ⭐⭐⭐⭐ | ⭐⭐ | 训练数据隐私泄露、不安全代码建议、许可证污染 |
| 5 | 工具权限三级模型 | ⭐⭐⭐ | ⭐⭐ | 只读（无需确认）→写入（需确认）→系统（需MFA+人工审批） |
| 6 | 间接注入 via 工具返回 | ⭐⭐⭐ | ⭐⭐⭐ | 工具返回值含隐藏指令污染Agent下一步推理，需消毒后送入上下文 |

### 知识巧记口诀
> 🎵 **Agent 安全核心**："工具分级权限控，沙箱隔离执行牢；返回值消毒防间接，人工确认高风险——Agent安全五道关"

> 🎵 **Copilot 三防**："内容排除防密钥，SAST扫描查漏洞；许可检查避污染，AI建议≠安全码"

---

> **Agent 安全是 LLM 安全的终极形态。当 LLM 从"说"变成"做"，安全防护必须从内容层延伸到执行层——权限最小化、沙箱隔离、人工确认三原则缺一不可。**

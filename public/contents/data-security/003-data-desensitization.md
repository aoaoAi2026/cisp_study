...
> **📘 文档定位**：CISP 考试数据安全技术内容 | 难度：⭐⭐⭐ | 预计阅读：20 分钟
> 数据脱敏与匿名化是个人信息保护的核心技术手段。本文从概念辨析、脱敏方法分类、动态/静态脱敏到 K-匿名模型，系统梳理数据脱敏技术体系。
...
> 🔑 **高分考点**：数据脱敏与匿名化是个人信息保护的技术核心。考试侧重脱敏与匿名化的概念辨析、脱敏方法分类、K-匿名模型。

| 考点 | 频次 | 核心记忆点 |
|:---|:---:|:---|
| 脱敏 vs 匿名化 | ⭐⭐⭐⭐⭐ | 脱敏可逆/不可逆保留格式，匿名化不可逆不可复原 |
| 脱敏方法 | ⭐⭐⭐⭐ | 遮盖/替换/加密/偏移/泛化/差分隐私 |
| 静态 vs 动态脱敏 | ⭐⭐⭐⭐ | 静态(ETL批处理)、动态(实时查询代理) |
| K-匿名 | ⭐⭐⭐ | 每条记录至少与 K-1 条不可区分 |
| 差分隐私 | ⭐⭐⭐ | 添加噪声使单条记录存在与否不影响统计结果 |

> 💡 **知识巧记**：脱敏六法记"遮替加偏泛差"——遮盖、替换、加密、偏移、泛化、差分隐私。静态动态记"静批动实"——静态批处理、动态实时代理。K-匿名记"K-1 混淆度"——每条与 K-1 条不可区分。

### 高分考点速查表

| 考察维度 | 关键结论 | 常见干扰项 |
|:---|:---|:---|
| 脱敏可逆性 | 部分脱敏方法可逆(加密)，部分不可逆(哈希) | "脱敏都是不可逆的" ❌ |
| 匿名化法律意义 | 匿名化后不再属于个人信息，不受个保法约束 | "脱敏就等于匿名化" ❌ |
| 动态脱敏 | 不改变原始数据，查询时实时处理 | "动态脱敏会修改原始数据" ❌ |
| K-匿名缺陷 | 无法抵御背景知识攻击、同质化攻击 | "K-匿名完全保护隐私" ❌ |
